...
tresult PLUGIN_API WaterStickController::initialize(FUnknown* context)
{
    ...
    parameters.addParameter(STR16("Comb Fade Time"), STR16("ms"), 0, 0.5,  // Default 25ms (0.5 normalized)
                           Vst::ParameterInfo::kCanAutomate, kCombFadeTime, 0,
                           STR16("Comb"));
...
tresult PLUGIN_API WaterStickController::getParamValueByString(Vst::ParamID id, Vst::TChar* string, Vst::ParamValue& valueNormalized)
{
    switch (id)
    {
        ...
        case kCombFadeTime:
        {
            // Parse millisecond values back to normalized format
            Steinberg::UString wrapper(string, tstrlen(string));
            char8 asciiBuffer[128];
            wrapper.toAscii(asciiBuffer, 128);

            // Try to parse as float with "ms" suffix
            char* endPtr;
